...
export default function AISommelier({ products, onAddProductDirectly, isOpen, onClose }: AISommelierProps) {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 'welcome',
      sender: 'chef',
      text: 'Olá! Sou o Chef virtual do Komy Sushi. 👨‍🍳✨ Estou aqui para sugerir as melhores combinações, explicar nossos ingredientes fresh, indicar harmonizações de bebidas ou ajudar você a montar o seu pedido perfeito. O que está com vontade de saborear hoje?',
  ...
  const quickQuestions = [
    "🔥 Qual o sushi mais vendido?",
    "🍣 Sugira um combinado especial",
    "🍣 Tem combinados de sushi premium?",
    "🍫 Qual sobremesa você recomenda?",
    "🥤 Sugira uma bebida para acompanhar",
  ];
  ...
  const handleSendMessage = async (text: string) => {
    ...
    try {
      ...
      const fallbackMsg: Message = {
        ...
        sender: 'chef',
        text: 'Desculpe, a nossa cozinha virtual está super movimentada agora! Se preferir, você pode explorar nosso cardápio usando as abas de categorias. Recomendo fortemente nosso **Hot Roll Premium** ou o maravilhoso **Combinado Especial**!',
        time: new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })
      };
