...
export default function CheckoutView({
  ...
  const isStep1Valid = () => {
    ...
    if (deliveryType === 'pickup') return isClientInfoOk;
            ...
            {deliveryType === 'delivery' && (
              <div className="space-y-4 animate-in fade-in duration-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {/* CEP */}
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-bold text-zinc-500">CEP</label>
                    <input
                      type="text"
                      value={address.cep}
                    ...
                    {cepError && (
                      <span className="text-[10px] text-red-500">{cepError}</span>
                    )}
                  </div>
                ...
                {needsChange && (
                  <div className="flex flex-col gap-1.5 animate-in zoom-in-95 duration-150 max-w-[200px]">
                    ...
                    {parseFloat(changeForValue.replace(',', '.')) <= total && (
                      <span className="text-[10px] text-red-500 font-medium">O valor deve ser maior do que o total (R$ {total.toFixed(2).replace('.', ',')})</span>
                    )}
                  </div>
                )}
              </div>
